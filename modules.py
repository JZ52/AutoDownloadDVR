...
def process_camera(store, cam, date_id, start_t, end_t):
    ...
    fragments = xml_builder.fetch_all_fragments(session, base_url, cam, start_t, end_t)
    ...
    for item in fragments:
        start_str = xml_builder.find_tag(item, 'startTime').text.replace('Z', '')
        end_str = xml_builder.find_tag(item, 'endTime').text.replace('Z', '')
        ...
        if not current_event:
            ...
            if gap_seconds <= settings.GAP_THRESHOLD_SEC:
    ...
    for event_idx, event_fragments in enumerate(events, 1):
        ...
        try:
            for i, (item, _, _) in enumerate(event_fragments, 1):
                uri_elem = xml_builder.find_tag(item, 'playbackURI')
                if uri_elem is None: continue

                uri = uri_elem.text
                chunk_name = os.path.join(settings.TEMP_DIR, f"{store['name']}_{cam}_{date_id}_E{event_idx}_{i:03d}.ps")
        ...
        final_filepath = os.path.join(settings.READY_DIR, final_filename)
        list_txt_path = os.path.join(settings.TEMP_DIR, f"list_{store['name']}_{cam}_{date_id}_E{event_idx}.txt")
        ...
        ff_cmd = ["ffmpeg"] + settings.CFG["FF_DEFAULT"][:4] + ["-i", list_txt_path] + settings.CFG["FF_DEFAULT"][4:] + [final_filepath]
        ...
        if result.returncode != 0 and ("sample rate not set" in result.stderr or "0 channels" in result.stderr):
            ...
            ff_cmd_fallback = ["ffmpeg"] + settings.CFG["FF_FALLBACK"][:4] + ["-i", list_txt_path] + settings.CFG["FF_FALLBACK"][4:] + [
                final_filepath]
